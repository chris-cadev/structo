...
func setupLogger(config MovementConfiguration) (MovementConfiguration, error) {
	timestamp := time.Now().Format("2006-01-02_15-04-05")
	logFilename := filepath.Join(config.OutputFolder, fmt.Sprintf(".organizer_%s.log", timestamp))

	logFile, err := os.OpenFile(logFilename, os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
		return MovementConfiguration{}, fmt.Errorf("failed to open log file %q: %w", logFilename, err)
	}
